...
bias_plan <- data.frame(N = rep(N, each = length(l_test)*50),
                        l = rep(rep(l_test, each = 50), length(N)),
                        iter = rep(rep(1:50, length(l_test)), length(N)))

burnin <- 5000
nthin <- 1
niter <- 70000*nthin+burnin


# ------- File management ------ ----

resDir <- paste0("./results/LMM2_pop30")
...
for(line in 1:nrow(bias_plan)){
  ...
  load(file = paste0("./data/Simul_pop30/simul_S", S, "_N", N, "_a", alpha, 
