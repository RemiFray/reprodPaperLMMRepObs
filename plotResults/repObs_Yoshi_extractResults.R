...
for(s in S){
  ...
  for(l in l_test){
    cat("\t", l, "\n")
      for(a in alpha){
        ...
        directory <- paste0("./results/Yoshi_S", s)
  ...
  write.csv(testSummaries,
            file=paste0("./results/noRep/", model, "_S", s, ".csv"), 
            row.names = F)
}


resS5 <- data.frame(model = rep(model, 120)) %>% 
  cbind(read.csv(paste0("./results/noRep/", model, "_S5.csv")) )
resS7 <- data.frame(model = rep(model, 120)) %>% 
  cbind(read.csv(paste0("./results/noRep/", model, "_S7.csv")) )
resS9 <- data.frame(model = rep(model, 120)) %>% 
  cbind(read.csv(paste0("./results/noRep/", model, "_S9.csv")) )

allres <-  resS5 %>% 
  rbind(resS7) %>% 
  rbind(resS9)

write.csv(allres, file=paste0("./results/noRep/", model,
                                   "_AllResults.csv"), row.names = F)


yoshi <- read.csv("results/noRep/Yoshi_AllResults.csv")
Mt <- read.csv("results/noRep/Mt_AllResults.csv")
LMM2 <- read.csv("results/noRep/LMM2_Pt_AllResults.csv")

dta <- LMM2 %>% 
  bind_rows(yoshi) %>% 
  bind_rows(Mt)

write.csv(dta, file=paste0("./results/noRep/AllModels_AllResults.csv"), 
          row.names = F)
